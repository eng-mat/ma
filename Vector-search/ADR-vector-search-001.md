...
All operations (index creation, serving, querying) remain fully private — **no internet egress** is required for Vector Search.
...
Example (not for Automation to run — this is pre-setup):Bashgcloud network-connectivity service-connection-policies create vectorsearch-policy-us-central1 \
...
If inside VPC-SC perimeter, ensure aiplatform.googleapis.com is allowed (recommended alignment with other AI services).
...
Get PSC internal IP from console post-deployment.
Preferred: Use SDK with psc_network param — auto-resolves.
...
Add pre-checks: policy existence, key existence, quotas.

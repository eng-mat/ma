...
- No internet egress required — all Vector Search traffic stays private via PSC.
...
Update internal wiki/Confluence with:
"For Vector Search PSC friendly names, use the standard DNS self-service tool with A records pointing to the PSC IP shown in the deployment console."
...
While DNS configuration is fully optional and post-deployment, the console can make the experience smoother:

Display the PSC internal IP clearly after deployment (with a copy button)
Show a suggestion: "To create a friendly name (e.g. vs-myapp-us-central1.mycompany.internal), use the DNS self-service portal: [link]"

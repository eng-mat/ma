...
Important:  
- Internal-only agents do **not** need to configure the SWP — traffic stays private via PSC-I.  
- Only agents that require outbound/internet access will point to this SWP (via explicit HTTPS_PROXY config).
...
- Private DNS zone exists in host project (e.g. agentengine-private-zone or mycompany.internal)
...
If Zscaler requires authentication, add --upstream-proxy-username and --upstream-proxy-password flags (or use Secret Manager)
...
Replace <SWP_GATEWAY_INTERNAL_IP> with the actual internal IP from Step 7
Naming convention: agentengine-swp-<env>-<region>.mycompany.internal (e.g. nonprod-us-central1, prod-europe-west4)
This single hostname is used by all agents in that host/VPC/region — Automation pre-fills it in console

...
- Proxy setup documented separately for Cloud Engineering to implement
...
- Proxy setup: See separate document Proxy-Setup-AgentEngine-OneTime.md (Cloud Engineering handles post-VM/MIG steps, MIG health checks, exact config for HTTPS-only on 3128)
...
- Proxy Setup: See Proxy-Setup-AgentEngine-OneTime.md (Cloud Engineering to handle VM/MIG deployment, HTTPS-only config on port 3128, MIG health checks, etc.).
- This design supports all Agent Engine use cases while enforcing private connectivity, CMEK, and strict HTTPS-only Zscaler egress.
